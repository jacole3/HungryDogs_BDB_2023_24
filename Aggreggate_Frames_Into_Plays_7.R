...
StatsByPlay_MergedData <- MergedData %>% 
  ...
  summarize(Frames = n(), down = max(down), distance = max(ydstogo), Team = max(club),
            ...
            quarter = max(quarter), PosTeam = max(possessionTeam), DefTeam = max(defensiveTeam),
            ...
            DropbackType = max(passResult), pass_location = max(pass_location),
            ...
            interception = max(interception), pass_touchdown = max(pass_touchdown), 
            CompProb = max(cp), CPOE = max(cpoe), Dropback = max(pass),
            ...
            WP = max(wp), DefenseWP = max(def_wp), WPA = max(wpa), DefWPA = max(DefWPA),
            ...
            OffTeam_FumbleLost = max(fumble_lost),
...
StatsByPlay_Scrambles <- Scrambles_Merged %>% 
  ...
  summarize(Frames = n(), down = max(down), distance = max(ydstogo), Team = max(club),
            ...
            game_seconds_remaining = max(game_seconds_remaining),
            sp = max(sp), goal_to_go = max(goal_to_go), no_huddle = max(no_huddle),
            DropbackType = max(passResult), Dropback = max(pass),
            TeamQBHit = max(qb_hit),
            run_location = max(run_location), run_gap = max(run_gap),
            td_team = max(td_team), td_player_name = max(td_player_name),
            td_player_id = max(td_player_id), PosTeamScore = max(posteam_score),
